...
- `pc | web | app | wechat` 端
- 获取指定的问答，如果 `isSecret=1` 仅创建者本人可获取
...
| pageSize | number | 否 | 每页页数，默认 10 |
| keyword | string | 否 | 关键字 |
| isSecret | string | 否 | 是否为私密问答，1 是 0 否，其中 1 只对自己发布的文章才有权限查看 |
...
- `pc | web | app | wechat` 端
- 获取问答列表，如果 `isSecret=1` 仅创建者本人可获取
...
| pageSize | number | 否 | 每页页数，默认 10 |
| keyword | string | 否 | 关键字 |
| isSecret | string | 否 | 是否为私密问答，1 是 0 否，其中 1 只对自己发布的问答才有权限查看 |
