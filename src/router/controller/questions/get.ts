...
export const getQuestionList = async (options: QuestionListParams): Promise<QuestionListReturn> => {
  ...
  const sql2 = `SELECT t1.id, ${orderParams.orderValid} t1.content,  t1.classify, t1.is_draft, t1.is_secret, t1.is_top, t1.sort, t1.create_user, t1.create_time, t1.update_time, t1.terminal, t1.remarks, t5.id AS is_like, (SELECT COUNT(t6.id) FROM likes t6 WHERE t6.target_id = t1.id AND t1.id) AS like_count, t7.id AS is_collection, (SELECT COUNT(t8.id) FROM collections t8 WHERE t8.target_id = t1.id AND t1.id) AS collection_count, (SELECT COUNT(t9.id) FROM comments_first t9 WHERE t9.target_id = t1.id AND t1.id) AS comment_count1, (SELECT COUNT(t10.id) FROM comments_second t10 WHERE t10.comment_first_target_id = t1.id AND t1.id) AS comment_count2 FROM questions t1 LEFT JOIN users t4 ON t1.create_user = t4.id LEFT JOIN likes t5 ON (t1.id = t5.target_id AND t5.create_user = ?) LEFT JOIN collections t7 ON (t1.id = t7.target_id AND t7.create_user = ?) ${whereSQL} ORDER BY ${orderSql}  LIMIT ?, ?`
