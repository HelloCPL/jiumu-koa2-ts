...
export const getLoginInfoList = async (params: LoginInfoListParams): Promise<LoginInfoListReturn> => {
  ...
  const sql2 = `SELECT t1.id, t1.user_id, t2.username, t1.user_agent, t1.create_time, t1.terminal FROM login_info t1 LEFT JOIN users t2 ON t1.user_id = t2.id WHERE user_id = ?  ORDER BY update_time DESC LIMIT ?, ?`
