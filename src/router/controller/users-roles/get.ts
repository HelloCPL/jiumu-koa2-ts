...
// 获取指定权限关联的所有角色
export const doUserRoleGetAllRoleByUserId = async (
	ctx: Context,
	next: Next
) => {
...
// 获取指定角色关联的所有用户
export const doUserRoleGetAllUserByRoleId = async (
	ctx: Context,
	next: Next
) => {
...
export const getAllRoleByUserId = async (
	...
	if (options.all) {
		const sql = `SELECT t1.id As relevance_id, t2.id, t2.code, t2.label, t2.sort, t2.configurable, t2.create_time, t2.update_time, t2.terminal, t2.remarks FROM users_roles t1 LEFT JOIN roles t2 ON t1.role_id = t2.id WHERE t1.user_id = ? ORDER BY t2.sort, t2.update_time DESC`
		...
		const sql2 = `SELECT t1.id As relevance_id, t2.id, t2.code, t2.label, t2.sort, t2.configurable, t2.create_time, t2.update_time, t2.terminal, t2.remarks FROM users_roles t1 LEFT JOIN roles t2 ON t1.role_id = t2.id WHERE t1.user_id = ? ORDER BY t2.sort, t2.update_time DESC LIMIT ?, ?`
 ...
 */
export const getAllUserByRoleId = async (
	options: UserRoleByRoleIdParams
	...
	let sql2: string
	if (options.simple === '1') {
		sql2 = `SELECT t1.id As relevance_id, t2.id, t2.phone, t2.username, t2.create_time, t2.update_time, t2.terminal FROM users_roles t1 LEFT JOIN users t2 ON t1.user_id = t2.id WHERE t1.role_id = ? ORDER BY t2.update_time DESC LIMIT ?, ?`
	} else {
		sql2 = `SELECT t1.id As relevance_id, t2.id, t2.phone, t2.username, t2.sex, t3.label as sex_label, t2.birthday, t2.avatar, t2.professional, t2.address, t2.create_time, t2.update_time, t2.terminal, t2.remarks FROM users_roles t1 LEFT JOIN users t2 ON t1.user_id = t2.id LEFT JOIN tags t3 ON t2.sex = t3.code  WHERE t1.role_id = ? ORDER BY t2.update_time DESC LIMIT ?, ?`
	}
	const res: any = await execTrans([
