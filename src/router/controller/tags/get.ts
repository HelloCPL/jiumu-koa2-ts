...
export const doTagGetAllSelf = async (ctx: Context, next: Next) => {
	const data = <TagListReturnOptions>(
		await getAllTagByUserId({ userId: ctx._user.id, all: true })
	)
 ...
 * 获取指定的某个标签，返回对象或null
 */
export const getTagByCode = async (
	code: string
): Promise<TagOptions | null> => {
...
export const getTagByParentCode = async (
	...
	if (global._results._tags && global._results._tags.length) {
		...
		if (userId) {
			...
			sqlLeft =
				'LEFT JOIN users_tags t3 ON (t3.user_id = ? AND t3.tag_code = t1.code)'
			data.push(userId)
		}
