...
import {
	PermissionReturnOptions,
	PermissionOptions
} from '../permissions/interface'
import { RoleReturnOptions } from '../roles/interface'
import {
	RolePermissionByRoleIdParams,
	RolePermissionByPermissionIdParams
} from './interface'
...
// 获取指定角色关联的所有权限
export const doRolePermissiongetAllPermissionByRoleId = async (
	ctx: Context,
	next: Next
) => {
...
// 获取指定权限关联的所有角色
export const doRolePermissionGetAllRoleByPermissionId = async (
	ctx: Context,
	next: Next
) => {
...
// 获取指定用户关联的所有权限
export const doRolePermissiongetAllPermissionByUserId = async (
	ctx: Context,
	next: Next
) => {
...
// 获取指定权限关联的所有用户
export const doRolePermissionGetAllUserByPermissionId = async (
	ctx: Context,
	next: Next
) => {
...
export const getAllPermissionByRoleId = async (
	...
	const sql2 = `SELECT t1.id As relevance_id, t2.id, t2.code, t2.label, t2.href, t2.sort, t2.configurable, t2.create_time, t2.update_time, t2.terminal, t2.remarks FROM roles_permissions t1 LEFT JOIN permissions t2 ON t1.permission_id = t2.id WHERE t1.role_id = ? ORDER BY t2.sort, t2.update_time DESC LIMIT ?, ?`
...
export const getAllRoleByPermissionId = async (
	...
	const sql2 = `SELECT t1.id As relevance_id, t2.id, t2.code, t2.label, t2.sort, t2.configurable, t2.create_time, t2.update_time, t2.terminal, t2.remarks FROM roles_permissions t1 LEFT JOIN roles t2 ON t1.role_id = t2.id WHERE t1.permission_id = ? ORDER BY t2.sort, t2.update_time DESC LIMIT ?, ?`
	const res: any = await execTrans([
...
export const getAllUserByPermissionId = async (
	...
	options.pageSize = options.pageSize || 10
	const pageNo = (options.pageNo - 1) * options.pageSize
	const sql1 = `SELECT COUNT(t1.id) AS total FROM users_roles t1 WHERE t1.role_id IN (SELECT t2.role_id FROM roles_permissions t2 WHERE t2.permission_id = ?) GROUP BY t1.user_id`
	let sql2: string
	if (options.simple === '1') {
		sql2 = `SELECT t3.id, t3.phone, t3.username, t3.create_time, t3.update_time, t3.terminal FROM users_roles t1 LEFT JOIN users t3 ON t1.user_id = t3.id  WHERE t1.role_id IN (SELECT t2.role_id FROM roles_permissions t2 WHERE t2.permission_id = ?) GROUP BY t1.user_id ORDER BY t3.update_time DESC LIMIT ?, ?`
	} else {
