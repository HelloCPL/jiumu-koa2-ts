...
async function _handleCommentList(data: CommentOptions[], userId: string, flag: number, type?: string) {
  for (let i = 0, len = data.length; i < len; i++) {
    ...
    else if (flag === 2) data[i].is_target_user = await _getTargetCreateUser(data[i].target_id, <string>data[i].target_type, data[i].create_user)
    data[i].flag = flag
  }
}
...
async function _getTargetCreateUser(targetId: string, type: string, userId: string): Promise<string> {
  ...
  if (res && res.length && res[0]['create_user'] === userId)
    return '1'
  return '0'
}