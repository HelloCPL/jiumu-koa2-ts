...
// 获取指定特殊标签关联的所有用户
export const doUserTagGetAllUserByTagCode = async (
	ctx: Context,
	next: Next
) => {
...
export const getAllTagByUserId = async (
	...
	if (options.all) {
		...
		const sql2 = `SELECT t1.id As relevance_id, t2.id, t2.parent_code, t3.label as parent_label, t2.code, t2.label, t2.sort, t2.configurable, t2.create_time, t2.update_time, t2.terminal, t2.remarks FROM users_tags t1 LEFT JOIN tags t2 ON t1.tag_code = t2.code LEFT JOIN tags t3 ON t2.parent_code = t3.code WHERE t1.user_id = ? ORDER BY t2.sort, t2.update_time DESC LIMIT ?, ?`
 ...
 */
export const getAllUserByTagCode = async (
	options: UserTagByTagCodeParams
	...
	let sql2: string
	if (options.simple === '1') {
		sql2 = `SELECT t1.id As relevance_id, t2.id, t2.phone, t2.username, t2.create_time, t2.update_time, t2.terminal FROM users_tags t1 LEFT JOIN users t2 ON t1.user_id = t2.id   WHERE t1.tag_code = ? ORDER BY t2.update_time DESC LIMIT ?, ?`
	} else {
