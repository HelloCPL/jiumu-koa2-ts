...
export const doNovelNoteAdd = async (ctx: Context, next: Next) => {
  ...
  const paramsData = await validateRange([
    { value: params.isSecret, range: ['1', '0'], default: '1' }
  ], true)
  ...
  const data = [getUuId(), params._target, params.title, params.content, params.classify, sort, paramsData[0], ctx._user.id, currentTime, currentTime, Terminal[ctx._terminal], params.remarks]
