...
export const getMenuOne = async (id: string): Promise<MenuOptions | null> => {
  const sql: string = `SELECT t1.id, t1.parent_code, t2.label AS parent_label, t1.code, t1.configurable, t1.label, t1.sort, t1.create_time, t1.update_time, t1.remarks FROM menus t1 LEFT JOIN menus t2 ON t1.parent_code = t2.code WHERE t1.code = ? OR t1.id = ?`
  const data = [id, id]
...
export const getMenuByParentCode = async (parentCode: string, userId?: string, roleId?: string): Promise<MenuListOptions[]> => {
  if (global._results._menus && global._results._menus.length) {
    ...
    const sql = `SELECT DISTINCT t1.id, t1.parent_code, t2.label AS parent_label, t1.code, t1.label, t1.sort, t1.configurable, t1.create_time, t1.update_time, ${sqlRoleId} ${sqlUserId} t1.remarks FROM menus t1 LEFT JOIN menus t2 ON t1.parent_code = t2.code ${sqlRoleIdLeft} ${sqlUserIdLeft}`
