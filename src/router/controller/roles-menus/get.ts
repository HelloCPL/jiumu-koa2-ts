...
export const getAllMenuByRoleId = async (
	...
	if (isTree) {
		...
		const sql2 = `SELECT t2.id, t2.parent_code, t3.label AS parent_label, t2.code, t2.label, t2.sort, t2.configurable, t2.create_time, t2.update_time, t2.terminal, t2.remarks FROM roles_menus t1 LEFT JOIN menus t2 ON t1.menu_id = t2.id LEFT JOIN menus t3 ON t2.parent_code = t3.code WHERE t1.role_id = ? ORDER BY t2.sort, t2.update_time DESC LIMIT ?, ?`
...
export const getAllRoleByMenuId = async (
	...
	const sql2 = `SELECT t2.id, t2.code, t2.label, t2.sort, t2.configurable, t2.create_time, t2.update_time, t2.terminal, t2.remarks FROM roles_menus t1 LEFT JOIN roles t2 ON t1.role_id = t2.id WHERE t1.menu_id = ? ORDER BY t2.sort, t2.update_time DESC LIMIT ?, ?`
	const res: any = await execTrans([
